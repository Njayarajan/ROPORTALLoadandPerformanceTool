    ...
    <div className="space-y-6">
      ...
      <HelpHighlighter tourId="dashboard" step={1} activeTourId={activeHelpTour} activeStep={currentHelpStep}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {/* Row 1 */}
          <StatCard icon={<UsersIcon className="w-6 h-6"/>} title="Peak Concurrent Users" value={config?.users.toLocaleString() ?? 'N/A'} tooltipText="The maximum number of concurrent virtual users active during the test." />
          ...
          {/* Row 2 */}
          <StatCard icon={<ClipboardDocumentListIcon className="w-6 h-6"/>} title={config?.runMode === 'iterations' ? 'Total Iterations' : 'Total Requests'} value={stats?.totalRequests.toLocaleString() ?? 'N/A'} tooltipText="The total number of requests sent during the test. For iteration tests, this should match the configured number." />
